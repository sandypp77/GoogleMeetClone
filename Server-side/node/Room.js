...
module.exports = class Room {
  constructor(room_id, worker, io) {
    this.id = room_id;
    //get mediacodecs from config
    const mediaCodecs = config.mediasoup.router.mediaCodecs;
    //create router in constructor
    worker
      .createRouter({
        mediaCodecs,
      })
  ...
  //function get list of producer from peers
    ...
    this.peers.forEach((peer) => {
      peer.producers.forEach((producer) => {
        producerList.push({
          ...
          type: peer.producersType.get(producer.id)
        });
      });
    });
    return producerList;
  ...
  async produce(socket_id, producerTransportId, rtpParameters, kind, type) {
    ...
    return new Promise(
      async function (resolve, reject) {
        let name = this.peers.get(socket_id).name;
        let usersId = this.peers.get(socket_id).usersId;
